...
async function summarizeBreakoutCondition(ctx: ExtensionContext, mode: LoopMode, condition?: string): Promise<string> {
...
export default function loopExtension(pi: ExtensionAPI): void {
	...
	function triggerLoopPrompt(ctx: ExtensionContext): void {
		const loopPrompt = loopState.prompt;
		if (!loopState.active || !loopState.mode || !loopPrompt) return;
