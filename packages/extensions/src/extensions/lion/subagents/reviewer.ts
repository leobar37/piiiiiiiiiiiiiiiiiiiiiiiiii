...
export async function runReviewerDelegation(options: {
	...
	const taskId = `${options.task.id}-reviewer-${options.attempt}`;
	const task: DelegationTask = {
		id: taskId,
		definition: "reviewer",
		description: `Review ${options.task.id}: ${options.task.title}`,
