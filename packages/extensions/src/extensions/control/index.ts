...
import { type Api, complete, type Model, StringEnum, type TextContent, type UserMessage } from "@earendil-works/pi-ai";
import type {
	ExtensionAPI,
	ExtensionContext,
	MessageRenderer,
					...
					if (msg.type === "response") {
						if (msg.command === command.type) {
							...
							if (!waitForEvent) {
								...
								socket.end();
								if (!response) {
									reject(new Error("Did not receive a response"));
									return;
								}
								resolve({ response });
								return;
...
function registerControlSessionsCommand(pi: ExtensionAPI): void {
	pi.registerCommand("control-sessions", {
		...
		handler: async (_args, ctx) => {
			...
			const content =
				sessions.length === 0 ? "No live sessions found." : `Controllable sessions:\n${lines.join("\n")}`;
