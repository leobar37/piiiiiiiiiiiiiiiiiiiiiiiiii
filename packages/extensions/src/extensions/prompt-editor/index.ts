...
async function loadModesFile(filePath: string, ctx: ExtensionContext, pi: ExtensionAPI): Promise<ModesFile> {
	try {
		...
		const modesRaw =
			parsed.modes && typeof parsed.modes === "object" ? (parsed.modes as Record<string, unknown>) : {};
		...
		handler: async (args, ctx) => {
			...
			if (tokens[0] === "store") {
				...
				const target =
					tokens[1] ??
					(ctx.hasUI
						? await ctx.ui.select("Store current selection into mode", orderedModeNames(runtime.data.modes))
						: null);
				if (!target) return;
